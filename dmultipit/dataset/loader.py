...
def load_MSKCC_multimoda(clinical_file,
                         radiomics_file,
                         pathomics_file,
                         omics_file,
                         pdl1_file,
    ...
    if clinical_features is not None:
        ...
        datasets['clinicals'] = df_total[df_clinicals.columns].drop(columns=['os_int', 'pfs', 'label', 'pfs_censor'],
                                                                    errors='ignore')
    ...
    for modality in order:
        if modality.split('_')[0] == 'radiomics':
            if not rad:
                assert datasets["radiomics"] is not None, "order specifies a modality but the input file for loading " \
